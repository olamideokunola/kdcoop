package org.ofbiz.cooperativefunds.coopPortal;
...
public class PortalEvents {
	public static final String module = PortalEvents.class.getName();

	public static String getContributionAccountStatementLines(HttpServletRequest request, HttpServletResponse response) {
		...
		String finAccountId = (String) ctx.get("finAccountId");
		
		String fromDateStr = (String) ctx.get("fromDate");
		String toDateStr = (String) ctx.get("toDate");
    	
    	Timestamp fromEntryDate = null;
    	Timestamp thruEntryDate = null;
    	
		try {
			fromEntryDate = (Timestamp) ObjectType.simpleTypeConvert(fromDateStr, "Timestamp", null, locale);
			thruEntryDate = (Timestamp) ObjectType.simpleTypeConvert(toDateStr, "Timestamp", null, locale);
		...
		/** Get FinAccount using fundId and Party */
		
		GenericValue finAccount = null;
    	
		try {
			finAccount = delegator.findOne("FinAccount", UtilMisc.toMap("finAccountId", finAccountId), false);
			Debug.logInfo("My Log: ownerPartyId: " + partyId + " finAccountId: " + finAccountId, null);
		...
		/** Get statement lines from FinAccountTrans */
    	Map<String, Object> getFinAccountTransListAndTotals = UtilMisc.toMap("finAccountId", finAccountId, "userLogin", userLogin, "fromEntryDate", fromEntryDate, "thruEntryDate", thruEntryDate);
    	Map<String, Object> finAccountTransListAndTotals = null;
		
    	try {
    		finAccountTransListAndTotals = dispatcher.runSync("getFinAccountTransListAndTotals", getFinAccountTransListAndTotals);
    	...
    	List<GenericValue> finAccountTransList = new FastList<GenericValue>();
    	finAccountTransList = (List<GenericValue>) finAccountTransListAndTotals.get("finAccountTransList");
    	
    	//ctx.put("finAccountTransList", finAccountTransList);
		request.setAttribute("finAccountTransList", finAccountTransList);
		request.setAttribute("grandTotal", finAccountTransListAndTotals.get("grandTotal"));
		
		Debug.logInfo("My Log: finAccountTransListAndTotals: " + finAccountTransListAndTotals, null);
	...
	public static String calculateLoanRepayment(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    String repaymentAmountStr = (String) ctx.get("fundId");
